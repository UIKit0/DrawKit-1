...
- (void)					addColorStop:(DKColorStop*) stop
{
	...
	{
		[[NSNotificationCenter defaultCenter] postNotificationName:kDKNotificationGradientWillAddColorStop object:self];
								...
								startRadius:(CGFloat) sr endingAtPoint:(NSPoint) ep endRadius:(CGFloat) er
{
	...
	switch([self gradientType])
	{
		case kDKGradientTypeLinear:
			shader = [self newLinearShaderForStartingPoint:sp endPoint:ep];
			CGContextDrawShading( context, shader );
			CGShadingRelease(shader);
			break;

		case kDKGradientTypeRadial:
			shader = [self newRadialShaderForStartingPoint:sp startRadius:sr endPoint:ep endRadius:er];
			CGContextDrawShading( context, shader );
			CGShadingRelease(shader);
			break;
...
- (void)				encodeWithCoder:(NSCoder*) coder
{
	...
	[coder encodeDouble:m_gradAngle forKey:@"gradientAngle"];
	[coder encodeInteger:m_gradType forKey:@"gradientType"];
